...
let displayProducts = ((products) => {
    ...
    const rows = products.map(product => {
        ...
        if (showUpdate) {
            row += `<td><button id="${product._id}" class="btn btn-sm btn-outline-primary btn-update-product"
                    data-bs-toggle="modal" data-bs-target="#ProductFormDialog">
                    <span class="bi bi-pencil-square" data-toggle="tooltip" title="Edit Product">
                    </span></button></td>`;
        }
        // if user has permission to delete - add the delete button
        if (showDelete) {
            row += `<td><button id="${product._id}" class="btn btn-sm btn-outline-danger btn-delete-product">
